...
var fullURL = "http://tripplanning.translink.ca/hiwire?.a=iNextBusFind&ShowTimes=1&PublicNum=" + busStop + "&FromHourDropDown=" + hr + "&FromMinuteDropDown=" + min + "&FromMeridiemDropDown=" + meri + "&SB=Find+my+next+bus&.a=iTripPlanning";
	...
	httpRequest.onreadystatechange = function() {
		...
		{
			var pageHTML = httpRequest.responseText;
			var tableIndex = pageHTML.indexOf('<strong>Next Bus Times By Route</strong>');
			var tableHTML = pageHTML.substr(tableIndex);
			var busIndex = tableHTML.indexOf(busNum);
			var busHTML = tableHTML.substr(busIndex + busNum.length + 3);
			...
			var busTimes = busNoTags.substr(busNum.length);
			//strip spaces
			busTimes = busTimes.replace(/\s/g, "");
			console.log(busTimes);
			//split into array delimited by m
			var timesArr = busTimes.split("m");
			busTimes = "";
			for (i=0; i < timesArr.length; i++){
				busTimes = busTimes + " " + timesArr[i];
				if(i < timesArr.length - 1)
					busTimes = busTimes + "m";
			}
			/*var busT_panel = document.getElementById('bustime_panel');
			if(busNoTags.length)
				busT_panel.label = "Stop#" + busStop + ", " + busNum + ": " + busTimes;
			else
				busT_panel.label = "Error loading times";*/
		}
	}	
}
